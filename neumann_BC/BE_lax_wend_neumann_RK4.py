...
def g(u, delta_x, delta_t, viscosity, umatplus, umatminus, fmatplus, fmatminus, space_mat, a, b, added_vector):
    u_plus_half = neumann_flux_half_step(umatplus, fmatplus, delta_t, delta_x, u, 'plus', a, b)
    u_minus_half = neumann_flux_half_step(umatminus, fmatminus, dt, dx, u_n, 'minus', a, b)
...
def neumann_flux_half_step(mat1, mat2, delta_t, delta_x, u, flag, a, b):
    f_vec = compute_f(u)
    flux_coeff = 1 / 2 * delta_t / delta_x

    on_vec = np.zeros(len(u))  # "on" the edge of the boundary
    off_vec = np.zeros_like(on_vec)  # just "off" the edge of the boundary
    if flag == 'plus':
        on_vec[-1] = b * delta_x * (1 - delta_t * b)
        off_vec[-2] = - b * delta_t * u[-2]

    elif flag == 'minus':
        on_vec[0] = a * delta_x * (delta_t * a - 1)
        off_vec[1] = -delta_t * a * u[1]

    return 1 / 2 * mat1 @ u + flux_coeff * mat2 @ f_vec + on_vec + off_vec
...
for nu in nu_list:
    ...
    spatial_matrix[0, 1] = 2
    spatial_matrix[-1, -2] = 2

    # This next vector is specifically for Neumann boundary conditions.
    # See notes for more information.
    u_boundary_vec = np.zeros(m)
    u_boundary_vec[0] = -alpha
    u_boundary_vec[-1] = beta

    boundary_add_vector = 2 * nu / dx * u_boundary_vec
    ...
    for i in range(0, n):
        if i > 0:
            ...
            k1 = g(u_n, dx, dt, nu, u_half_matrix_plus, u_half_matrix_minus, flux_matrix_plus, flux_matrix_minus, spatial_matrix, alpha, beta, boundary_add_vector)
            k2 = g(u_n + k1 * dt / 2, dx, dt, nu, u_half_matrix_plus, u_half_matrix_minus, flux_matrix_plus, flux_matrix_minus, spatial_matrix, alpha, beta, boundary_add_vector)
            k3 = g(u_n + k2 * dt / 2, dx, dt, nu, u_half_matrix_plus, u_half_matrix_minus, flux_matrix_plus, flux_matrix_minus, spatial_matrix, alpha, beta, boundary_add_vector)
            k4 = g(u_n + k2 * dt, dx, dt, nu, u_half_matrix_plus, u_half_matrix_minus, flux_matrix_plus, flux_matrix_minus, spatial_matrix, alpha, beta, boundary_add_vector)
        ...
        if i % 25 == 0:
            ...
            # sns.scatterplot(x=xs, y=ans_array[i, :], label='Numerical Solution',marker='o',s=2.5)
            plt.xlabel('x')
            plt.ylabel('u(x,t)')
            plt.legend(loc='upper left')
            plt.ylim(0, 1.5)
            plt.title(
                f"Burgers' Equation with Neumann Boundary Conditions\n" + fr'$\nu={nu}$; $T=[0,{T}]$; $\Delta x={dx}$; $\Delta t = {dt}$')
